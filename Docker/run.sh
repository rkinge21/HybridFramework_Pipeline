...
do
 ...
 '
    ...
    rm -f ContainerImageIdList2
    touch ContainerImageIdList2
    docker ps --no-trunc --format "{{.Image}}" |sort >ContainerImageIdList2
    ...
    if [ -s ToBeCleaned ]; then
      ...
      rm TobeCleanedwithTags
      echo "=> Filtering Images that are in use"
      for a in $(cat ToBeCleaned)
       do
          docker images --no-trunc --format "{{.ID}}%{{.Repository}}:{{.Tag}}" | grep $a |cut -d% -f2 >>TobeCleanedwithTags
       done
      sort -u TobeCleanedwithTags -o TobeCleanedwithTags
      docker ps --no-trunc --format "{{.Image}}" |sort -u >ContainerImageTagList
      comm -23  TobeCleanedwithTag ContainerImageTagList  >TobeCleanedFinal
      echo "=> Start to clean $(cat ToBeCleanedFinal | wc -l) images"
      docker rmi $(cat TobeCleanedFinal)
